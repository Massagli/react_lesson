...
import { getRelativeTimeFormat, isRelativeTimeFormatSupported, } from './intl.js';
...
function formatRelativeTimeToPartsFactory() {
    if (!isRelativeTimeFormatSupported) {
        return (value, unit, locales, options) => {
            ...
            const numberFormat = convertToNumberFormat(value, unit, options);
            const numberFormatParts = formatNumberToParts(numberFormat.value, locales, numberFormat.options);
            ...
            return numberFormatParts.map((part) => {
                if (part.type === 'integer') {
                    return Object.assign(Object.assign({}, part), { unit: numberFormat.options.unit });
                }
                if (part.type === 'unit') {
                    return Object.assign(Object.assign({}, part), { type: 'literal' });
                }
                return part;
            });
        };
...
function convertToNumberFormat(value, unit, options) {
    const style = 'unit';
    const unitDisplay = (options === null || options === void 0 ? void 0 : options.style) || 'long';
    ...
    const singularUnit = numberFormatUnits.find((supportedUnit) => unit.startsWith(supportedUnit));
