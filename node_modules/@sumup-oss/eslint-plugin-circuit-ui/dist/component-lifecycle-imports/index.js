...
exports.componentLifecycleImports = void 0;
...
exports.componentLifecycleImports = createRule({
    ...
    meta: {
        ...
        fixable: 'code',
        docs: {
            description: 'Components that have moved to a different stage in their lifecycle should be imported from the relevant path.',
            recommended: 'recommended',
        },
        messages: {
            refactor: '`{{name}}` has moved to a different stage in its lifecycle. Import it from `{{source}}` instead.',
        },
    },
    defaultOptions: [],
    create(context) {
        return mappings.reduce((visitors, config) => {
            ...
            return Object.assign(visitors, {
                [`ImportDeclaration:has(Literal[value="${from}"])`]: (node) => {
                    node.specifiers.forEach((specifier) => {
                        ...
                        const importStatement = node.importKind === 'type' ? 'import type' : 'import';
                        const importSpecifier = importedName === localName
                            ? importedName
                        ...
                        context.report({
                            ...
                            fix(fixer) {
                                ...
                                if (node.specifiers.length === 1) {
                                    ...
                                    fixes.push(fixer.replaceText(node, context.sourceCode
                                        .getText(node)
                                        .replace(importSpecifier, '')
                                        .replace(' ,', '')));
                                }
                                // Insert the new import
                                fixes.push(fixer.insertTextAfter(node, `${importStatement} { ${importSpecifier} } from '${to}';`));
                                return fixes;
                            },
                        });
                    });
