...
exports.renamedPackageScope = void 0;
...
exports.renamedPackageScope = createRule({
    ...
    meta: {
        ...
        fixable: 'code',
        docs: {
            description: 'Imports from Circuit UI packages should use the new `@sumup-oss` scope.',
            recommended: 'recommended',
        },
        messages: {
            refactor: '`{{from}}` has been renamed to `{{to}}`. Update the import to use `{{to}}` instead.',
        },
    },
    defaultOptions: [],
    create(context) {
        return mappings.reduce((visitors, { from, to }) => {
            ...
            function fixPackageName(node) {
                context.report({
                    node,
                    messageId: 'refactor',
                    data: { from, to },
                    fix(fixer) {
                        return fixer.replaceText(node, context.sourceCode.getText(node).replace(from, to));
                    },
                });
            }
            return Object.assign(visitors, {
                [`ImportDeclaration:has(Literal[value=/${escapedFrom}(?!-).*/])`]: (node) => {
                    const { source } = node;
                ...
                [`ImportExpression:has(Literal[value=/${escapedFrom}(?!-).*/])`]: (node) => {
                    const { source } = node;
                    if (source.type !== 'Literal') {
                        return;
                    }
                ...
                [`CallExpression:has(Identifier[name=/(mock|requireActual|importActual)/]):has(Literal[value=/${escapedFrom}(?!-).*/])`]: (node) => {
                ...
                [`TSImportType:has(Literal[value=/${escapedFrom}(?!-).*/])`]: (node) => {
                    const { argument } = node;
