...
exports.noDeprecatedCustomProperties = void 0;
const utils_1 = require("@typescript-eslint/utils");
const design_tokens_1 = require("@sumup-oss/design-tokens");
const DEPRECATED_CUSTOM_PROPERTIES = design_tokens_1.schema.filter(({ deprecation }) => Boolean(deprecation));
...
exports.noDeprecatedCustomProperties = createRule({
    ...
    meta: {
        ...
        messages: {
            deprecated: 'The `{{name}}` custom property has been deprecated. Use the `{{replacement}}` custom property instead.',
        },
    },
    defaultOptions: [],
    create(context) {
            ...
            Program(node) {
                context.sourceCode.getLines().forEach((line, index) => {
                    ...
                    while ((match = regex.exec(line)) !== null) {
                        const name = match[0];
                        const { replacement } = DEPRECATED_CUSTOM_PROPERTIES.find((token) => token.name === name).deprecation;
                        context.report({
                            node,
                            loc: {
                                start: {
                            ...
                            fix(fixer) {
                                return fixer.replaceText(node, context.sourceCode.getText(node).replace(name, replacement));
                            },
                        });
                    }
                });
